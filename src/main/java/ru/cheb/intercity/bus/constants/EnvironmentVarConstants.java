package ru.cheb.intercity.bus.constants;


/**
 * Class contain names of environment variables.
 */
public class EnvironmentVarConstants {

    public static final String busStationHostUrl = "BUS_STATION_SERVER_HOST_URL";
    public static final String botUsername = "TELEGRAM_BOT_USERNAME";
    public static final String botTokenEnv = "TELEGRAM_BOT_TOKEN";

}
